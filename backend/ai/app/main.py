...
async def lifespan(app: FastAPI):
    logger.info("ai service starting", port=8008)
    if settings.environment == "development":
        async with engine.begin() as conn:
            await conn.run_sync(Base.metadata.create_all)
        logger.info("ai tables created/verified")
...
app = FastAPI(
    title="nerve.idp — AI Co-pilot",
    description="AI ops co-pilot — Claude API + pgvector",
    version="2.0.0",
    lifespan=lifespan,
)
