...
async def lifespan(app: FastAPI):
    logger.info("observability service starting", port=8005)
    if settings.environment == "development":
        async with engine.begin() as conn:
            await conn.run_sync(Base.metadata.create_all)
        logger.info("observability tables created/verified")
...
app = FastAPI(
    title="nerve.idp — Observability",
    description="DORA metrics, cost intelligence, error budgets",
    version="2.0.0",
    lifespan=lifespan,
)
...
async def health():
    return {"status": "ok", "service": "observability", "version": "2.0.0"}
